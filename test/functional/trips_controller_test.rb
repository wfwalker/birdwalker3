...
class TripsControllerTest < ActionController::TestCase
  ...
  def test_show_by_date_2
    first_trip_date = trips(:trip_one).date
    get :show_by_date, :year => "2007", :month => "01", :day => "01"
  ...
  def test_show_as_ebird_record_format
    ...
    assert_equal 'text/plain', @response.content_type, "response should be plain text"
    assert_equal "Dodo,,,2,,First Place,,,01/01/2007,,FS,US,casual,,,Y,,,\nIvory-billed Woodpecker,,,X,,Second Place,,,01/01/2007,,SS,US,casual,,,Y,,,", @response.body, "ebird text should match"
  end
end
